...
CIRCLE = 2
RECTANGLE = 3
object_type = None#TRIANGLE
current_type = None#CIRCLE
...
# waypoint = [[(1.0740848757, -1.05159653387, 0.0), (0.0, 0.0, -0.631961673594, 0.774999640715)],  # First ar search
#             [(2.56334483958, -1.40539962141, 0.0), (0.0, 0.0, -0.70901709933, 0.705191288133)],  # Second ar search
#             [(2.31923830923, -1.8657422908, 0.0), (0.0, 0.0, 0.818550947409, 0.574433935711)],  # Shape search
#             [(3.6700010671, 0.212529648069, 0.0), (0.0, 0.0, 0.724713263952, 0.689050567848)],  # exit
#             ]
...
class detect_shape(smach.State):
    ...
    def execute(self, userdata):
        ...
        if current_type == object_type:
            ...
            while userdata.cur_time + rospy.Duration(0.7 / max_linear_vel) > rospy.Time.now():
                twist = Twist()
                twist.linear.x = max_linear_vel
                twist_pub.publish(twist)
...
class turning_back(smach.State):
    ...
    def execute(self, userdata):
        ...
        if userdata.cur_time + rospy.Duration((degree_ninty * 2) / max_rotate_vel) > rospy.Time.now():
            temp_twist = Twist()
            temp_twist.linear.x = 0
            temp_twist.angular.z = -max_rotate_vel
            twist_pub.publish(temp_twist)
...
class moving_loc2(smach.State):
    ...
    def execute(self, userdata):
        ...
        if counter_loc2 > 0:
            ...
            if userdata.cur_time + rospy.Duration(12) > rospy.Time.now():
