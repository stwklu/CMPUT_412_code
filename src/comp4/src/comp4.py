...
waypoint = [[(1.96824683061, -0.538598273645, 0.0), (0.0, 0.0, -0.555860436814, 0.831275631055)],  # First ar search
            [(3.40031019934, -0.349942778535, 0.0), (0.0, 0.0, -0.600280644879, 0.799789439405)],  # Second ar search
            [(3.08045856283, -0.697811777879, 0.0), (0.0, 0.0, 0.877188067771, 0.480146950172)],  # Shape search
            [(4.12683969007, 1.02565007911, 0.0), (0.0, 0.0, 0.815481506353, 0.578783131057)],  # exit
            ]
client = None
target_id = 2
box_id = 1
...
def get_back_pose():
    ...
    euler = tf.transformations.euler_from_quaternion(rot)
    new_rot = tf.transformations.quaternion_from_euler(0.0, 0.0, euler[2] + 3.1415926535)
...
class detect_target_pose(smach.State):
    ...
    def execute(self, userdata):
        global stop_line_flag, flag_line_flag, absolute_target_pose
        if relative_target_pose is None:
            if userdata.cur_time + rospy.Duration(3.1415926535 * 2 / max_rotate_vel) > rospy.Time.now():
                twist = Twist()
                ...
                client.wait_for_result()
                userdata.cur_time = rospy.Time.now()
                return 'detecting_again'

        else:
            distance = numpy.linalg.norm(numpy.array(
                [relative_target_pose.position.x, relative_target_pose.position.y, 0.0]))
            ...
            if distance > 3:
                ...
                if absolute_target_pose is None:
                    ...
                    if relative_target_pose.position.x > 0.5:
                        twist = Twist()
                        twist.linear.x = max_linear_vel
                        target_offset_y = relative_target_pose.postion.y

                        if abs(target_offset_y) > 0.05:
                            speed = target_offset_y * 1.5
                            twist.angular.z = copysign(max(0.01, min(0.6, abs(speed))), speed)
                        ...
                        client.send_goal(absolute_target_pose)
                        client.wait_for_result()
...
class detect_target_again(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['rotating', 'detect_box'],
                             input_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'],
                             output_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'])
...
class rotating_detect_box(smach.State):
    ...
    def execute(self, userdata):
        if relative_box_pose is None:
            ...
            client.send_goal(get_back_pose())
            client.wait_for_result()
            return 'moving'


class rotating_before_push(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['rotating', 'moving'],
                             input_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'],
                             output_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'])
...
class pushing_box(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['moving_towards', 'moving', 'stop'],
                             input_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'],
                             output_keys=['cur_time', 'cur_pose', 'cur_loc', 'cur_heading'])
    ...
    def execute(self, userdata):
        if relative_box_pose is None:
            ...
            x_disparity = cur_pos[0] - absolute_target_pose.target_pose.pose.position.x
            y_disparity = cur_pos[1] - absolute_target_pose.target_pose.pose.position.y

            distance = numpy.linalg.norm([x_disparity, y_disparity, 0])
            ...
            twist = Twist()
            twist.linear.x = max_linear_vel
            target_offset_y = relative_box_pose.postion.y

            print target_offset_y
...
class moving_shape(smach.State):
    ...
    def execute(self, userdata):
        global is_in_loction4
        if userdata.cur_time + rospy.Duration(2.0 / max_linear_vel) > rospy.Time.now():
            twist = Twist()
            twist.linear.x = max_linear_vel
            target_offset = 320 - shape_x

            if abs(target_offset) > 0.05:
                speed = target_offset * 1.5
                twist.angular.z = copysign(max(0.01, min(0.6, abs(speed))), speed)
                # twist.angular.z = 3
...
class main_controller():
    ...
    def image_callback(self, msg):
        ...
        lower_red = numpy.array([160, 195, 100])
        ...
        with self.sm:
            ...
            smach.StateMachine.add('detect_target_pose', detect_target_pose(),
                                   transitions={'rotating': 'detect_target_pose',
                                                'detecting_again': 'detect_target_again',
            ...
            smach.StateMachine.add('detect_target_again', detect_target_again(),
                                   transitions={'rotating': 'detect_target_again',
                                                'detect_box': 'rotating_detect_box'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
                                              'cur_loc': 'current_loc',
            ...
            smach.StateMachine.add('rotating_detect_box', rotating_detect_box(),
                                   transitions={'rotating': 'rotating_detect_box',
                                                'moving': 'rotating_before_push'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
                                              'cur_loc': 'current_loc',
                                              'cur_heading': 'current_heading'})

            smach.StateMachine.add('rotating_before_push', rotating_before_push(),
                                   transitions={'rotating': 'rotating_before_push',
                                                'moving': 'pushing_box'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
                                              'cur_loc': 'current_loc',
            ...
            smach.StateMachine.add('pushing_box', pushing_box(),
                                   transitions={'moving_towards': 'pushing_box',
                                                'moving': 'pushing_box',
                                                'stop': 'moving_center'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
            ...
            smach.StateMachine.add('moving_center', moving_center(),
                                   transitions={'arrived': 'detect_shape'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
                                              'cur_loc': 'current_loc',
                                              'cur_heading': 'current_heading'})
            ...
            smach.StateMachine.add('detect_shape', detect_shape(),
                                   transitions={'rotating': 'detect_shape',
                                                'moving': 'moving_shape'},
                                   remapping={'cur_time': 'current_time',
                                              'cur_pose': 'current_pose',
                                              'cur_loc': 'current_loc',
